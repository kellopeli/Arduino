...
//###### RSSI MODES ########
// Analog RSSI pin is Ch8 on v2 receivers. If you enable this function CH8 works as PWM RSSI output.
...
//thUndead's smooth RSSI calculation variables
...
unsigned char loop_counter = 0; // telemetry loop counter
    ...
    #define Serial_PPM_OUT_HIGH PORTB = _BV(5) //Serial PPM out on Servo 8
      ...
      #if defined(Analog_RSSI) // convert servo8 as PWM RSSI output
          #define Servo_Ports_LOW PORTB &= 0xE8; PORTD &= 0x17; // pulling down the servo outputs
      #else
